...
**Status on September 23, 2026:** Version 0.1.0 failed the [community directory review](https://community.obsidian.md/plugins/second-brain-lab). Version 0.1.1 fixes the blocking lifecycle issue; its automated scan completed without errors, and **Add to Obsidian** is active on the listing. A manual review request remains open. In Obsidian, open **Settings → Community plugins → Browse**, search for **Second Brain Lab 3D**, then install and enable it. The manual alternative is:
